	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu
	{
		...
		public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
		{
			...
			if (empty($item->attr_title)) {
				$anchor_atts['title'] = !empty($item->title) ? strip_tags($item->title) : '';
			} else {
			...
			if (isset($args->has_children) && $args->has_children && 0 === $depth && $args->depth > 1) {
				...
				$button_atts['data-toggle']   = 'dropdown';
				$button_atts['aria-haspopup'] = 'true';
				$button_atts['aria-expanded'] = 'false';
				$button_atts['class']         = 'dropdown-toggle nav-link';
				$button_atts['id']            = 'menu-item-dropdown-' . $item->ID;
			} else {
				$anchor_atts['href'] = !empty($item->url) ? $item->url : '#';
				// Items in dropdowns use .dropdown-item instead of .nav-link.
				if ($depth > 0) {
					$anchor_atts['class'] = 'dropdown-item';
				} else {
					$anchor_atts['class'] = 'nav-link';
				}
			}

			$anchor_atts['aria-current'] = $item->current ? 'page' : '';
			...
			$button_attributes = '';
			foreach ($button_atts as $attr => $value) {
				if (!empty($value)) {
					$value       		= ('href' === $attr) ? esc_url($value) : esc_attr($value);
			...
			if ('' !== $linkmod_type) {
				...
				$item_output .= self::linkmod_element_open($linkmod_type, $anchor_attributes);
			} else {
				if ($button_attributes) {
					// With no link mod type set this must be a standard <a> tag.
					$item_output .= '<button ' . $button_attributes . '></button>' . '<a ' . $anchor_attributes . '>';
				} else {
